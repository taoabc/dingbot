...
function destroyUserDB(): Promise<void> {
  return db.destroy();
}
...
async function getRealName(name: string, email?: string): Promise<string> {
  const employee = await findUser(name, email);
...
async function addUser(employee: Employee): Promise<PouchDB.Core.Response>;
...
async function getAllUsers(): Promise<Employee[]> {
  const emps: Employee[] = [];
...
async function deleteUser(): Promise<PouchDB.Core.Response | void> {}
